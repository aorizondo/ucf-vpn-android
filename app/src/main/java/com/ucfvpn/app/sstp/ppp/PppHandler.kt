...
object HDLCHandler {
     ...
     */
    private val fcsTable = intArrayOf(
     ...
     * Algorithm matches Python reference exactly.
     */
    fun fcs16(data: ByteArray): Int {
        var fcs = 0xFFFF
        for (b in data) {
            fcs = (fcs shr 8) xor fcsTable[(fcs xor (b.toInt() and 0xFF)) and 0xFF]
        }
        return fcs
    }
     ...
     * If rawPpp doesn't start with 0xFF 0x03, prepend it.
     * Append FCS-16 (little-endian), then escape and frame with 0x7E.
     */
    fun encode(rawPpp: ByteArray): ByteArray {
        // Ensure we have FF 03 header
        val data = if (rawPpp.isNotEmpty() && rawPpp[0] != 0xFF.toByte() || rawPpp.size < 2 || rawPpp[1] != 0x03.toByte()) {
            byteArrayOf(0xFF.toByte(), 0x03.toByte()) + rawPpp
        } else {
            rawPpp
        }
        ...
        // Self-check: fcs16(data + fcs_final_le) should be 0xf0b8
        val fcsLe = byteArrayOf(
            (fcsFinal and 0xFF).toByte(),
            ((fcsFinal shr 8) and 0xFF).toByte()
        )
        val goodFcsCheck = fcs16(data + fcsLe)
        if (goodFcsCheck == 0xf0b8) {
            Timber.d("FCS self-check OK (0xf0b8)")
        } else {
            Timber.e("FCS self-check FAILED: 0x%04x != 0xf0b8", goodFcsCheck)
        }

        Timber.d("Calculated FCS for %d bytes: %04x -> %04x (le: %s)", data.size, fcs, fcsFinal, fcsLe.toHexString())
...
class PPPHandler(
    ...
    var pppConnected: Boolean = false
        private set
     ...
     * Unwrap HDLC to get raw PPP, then send via callback.
     */
    fun onLwipOutput(hdlcFrame: ByteArray) {
        val rawFrame = HDLCHandler.decode(hdlcFrame)
        if (rawFrame.isNotEmpty()) {
            Timber.d("lwIP output raw PPP frame (%d bytes): %s", rawFrame.size, rawFrame.toHexString())
            sendCallback?.invoke(rawFrame)
        } else {
            Timber.d("lwIP output HDLC frame (yielded no raw PPP): %d bytes", hdlcFrame.size)
        }
    }
